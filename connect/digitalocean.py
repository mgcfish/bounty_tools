...
def add_args(parser):
    parser.add_argument("--droplet", help="Droplet ID")
    parser.add_argument("--listdroplets", help="List available DO Droplets", action="store_true")
    parser.add_argument("--setupdroplet", help="Create a Digital Ocean VM to execute on", action="store_true")
...
def parse_args(plugins, parser, config):
    ...
    if args.listdroplets:
        ...
        return get_droplet(args.droplet, config)

    elif args.setupdroplet:
